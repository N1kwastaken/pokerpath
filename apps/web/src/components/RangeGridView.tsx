...
export function RangeGridView({ grid, legend = true, highlight }: { grid: RangeGrid; legend?: boolean; highlight?: string }) {
  return (
    <div>
        ...
        {grid.cells.flatMap((row, r) =>
          row.map((cell, c) => {
            ...
            return (
              <div
                key={`${r}-${c}`}
                className={`flex aspect-square items-center justify-center rounded-[3px] text-[7px] font-bold leading-none sm:text-[9px] ${CELL_COLOR[cell.action]} ${on ? 'z-10 scale-110 outline outline-2 outline-gold ring-2 ring-gold/40' : ''}`}
