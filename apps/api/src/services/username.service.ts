...
import { isReservedDeveloperUsername } from '../lib/godmode.js';
...
export async function generateUniqueUsername(seed: string): Promise<string> {
  ...
  if (!isReservedDeveloperUsername(base) && !(await taken(base))) return base;
  ...
  for (let i = 0; i < 12; i++) {
    ...
    if (!isReservedDeveloperUsername(cand) && !(await taken(cand))) return cand;
  }
